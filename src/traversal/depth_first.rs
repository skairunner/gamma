...
mod into_table {
    ...
    #[test]
    fn c3() {
        let graph = DefaultGraph::try_from(vec![vec![1, 2], vec![0, 2], vec![1, 0]]).unwrap();
        let traversal = DepthFirst::new(&graph, 0).unwrap();

        assert_eq!(
            traversal.into_table(),
            (vec![0, 1, 2], vec![(0, 1), (1, 2), (2, 0)])
        )
    }

    #[test]
    fn s3() {
        let graph = DefaultGraph::try_from(vec![vec![1], vec![0, 2, 3], vec![1], vec![1]]).unwrap();
        let traversal = DepthFirst::new(&graph, 0).unwrap();

        assert_eq!(
            traversal.into_table(),
            (vec![0, 1, 2, 3], vec![(0, 1), (1, 2), (1, 3)])
        );
    }
}
...
mod tests {
    ...
    fn p3_inside() {
        ...
        assert_eq!(
            traversal.collect::<Vec<_>>(),
            vec![Step::new(1, 0, false), Step::new(1, 2, false)]
        );
    }

    #[test]
    fn p4() {
        let graph = DefaultGraph::try_from(vec![vec![1], vec![0, 2], vec![1, 3], vec![2]]).unwrap();
        let traversal = DepthFirst::new(&graph, 0).unwrap();
    ...
    fn flower_from_stalk() {
        let graph =
            DefaultGraph::try_from(vec![vec![1], vec![0, 2, 3], vec![1, 3], vec![2, 1]]).unwrap();
        let traversal = DepthFirst::new(&graph, 0).unwrap();
    ...
    fn flower_with_cut_in_branch() {
        let graph =
            DefaultGraph::try_from(vec![vec![1, 2], vec![0, 2], vec![1, 0, 3], vec![2]]).unwrap();
        let traversal = DepthFirst::new(&graph, 0).unwrap();
