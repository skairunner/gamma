...
mod tests {
    ...
    #[test]
    fn s3_tertiary() {
        let graph = DefaultGraph::try_from(vec![vec![1, 2, 3], vec![0], vec![0], vec![0]]).unwrap();
        let traversal = BreadthFirst::new(&graph, 0).unwrap();
    ...
    fn c4() {
        let graph =
            DefaultGraph::try_from(vec![vec![1, 3], vec![0, 2], vec![1, 3], vec![2, 0]]).unwrap();
        let traversal = BreadthFirst::new(&graph, 0).unwrap();
    ...
    fn diamond() {
        let graph =
            DefaultGraph::try_from(vec![vec![1, 2, 3], vec![2, 0], vec![0, 1, 3], vec![0, 2]])
                .unwrap();
