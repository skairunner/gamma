...
use crate::graph::{AppendableGraph, DefaultGraph, Graph};
...
impl<'a, G: Graph> Iterator for Components<'a, G> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        let traversal = DepthFirst::new(self.graph, *root).expect("root not found");
        let mut component = DefaultGraph::try_from(traversal).expect("traversal error");

        if component.is_empty() {
            component
                .add_node_with(*root)
                .expect("add root to empty graph");
        } else {
...
mod tests {
    ...
    #[test]
    fn p2_p1() {
        let graph = DefaultGraph::try_from(vec![vec![1], vec![0], vec![]]).unwrap();
        let components = components(&graph).collect::<Vec<_>>();
        let mut c1 = DefaultGraph::new();
        let mut c2 = DefaultGraph::new();
    ...
    fn p2_p2_p2() {
        let graph =
            DefaultGraph::try_from(vec![vec![1], vec![0], vec![3], vec![2], vec![5], vec![4]])
                .unwrap();
        let components = components(&graph).collect::<Vec<_>>();
        let mut c1 = DefaultGraph::new();
        let mut c2 = DefaultGraph::new();
    ...
    fn c3_p2() {
        let graph =
            DefaultGraph::try_from(vec![vec![1, 2], vec![0, 2], vec![0, 1], vec![4], vec![3]])
                .unwrap();
        let components = components(&graph).collect::<Vec<_>>();
        let mut c1 = DefaultGraph::new();
        let mut c2 = DefaultGraph::new();
